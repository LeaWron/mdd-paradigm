...
if gf.calibration.has_calibrated:
    ...
    # 打印模型 var_count（诊断）
    print(
        f"模型 var_count: X={gf.calibration.svr_x.getVarCount()}, Y={gf.calibration.svr_y.getVarCount()}"
    )
    print(1234567890)
else:
    print("模型加载失败，请检查路径。")
...
annotated_video_path = current_dir.joinpath(
    "output_eye_data/subject1_gaze_annotated.mp4"
)
...
while cap.isOpened():
    ...
    if face_info.status and face_info.can_gaze_estimation:
        ...
        print(
            f"feat.shpe: {feat.shape}, feat.dtype: {feat.dtype}, feat.size = {feat.size}"
        )
        if gaze_info.status:
            # 诊断：打印 features 信息（仅前几帧）
            if frame_count < 5:
                print(
                    f"帧 {frame_count}: features shape {gaze_info.features.shape}, dtype {gaze_info.features.dtype}"
                )

            # SVR 校准
            _, calibrated_coords = gf.calibration.predict(
            ...
            with open(output_csv, "a", newline="") as csvfile:
                ...
                writer.writerow(
                    [
                        timestamp,
                        gaze_info.raw_gaze_coordinates[0],
                        gaze_info.raw_gaze_coordinates[1],
                        calibrated_coords[0],
                        calibrated_coords[1],
                        filtered_coords[0],
                        filtered_coords[1],
            ...
            cv2.putText(
                vis, "raw", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2
            )
            cv2.putText(
                vis, "calib", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2
            )
            cv2.putText(
                vis, "filt", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2
            )
