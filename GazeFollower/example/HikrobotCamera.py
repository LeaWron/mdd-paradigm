        ...
        if (
            ...
            if self.auto_select:
                ...
                for i in range(deviceList.nDeviceNum):
                    ...
                    name = "".join(
                        chr(c)
                        for c in info.SpecialInfo.stUsb3VInfo.chModelName
                        if c != 0
                    )
                    sn = "".join(
                        chr(c)
            ...
            devInfo = cast(
                deviceList.pDeviceInfo[idx], POINTER(MV_CC_DEVICE_INFO)
            ).contents
            ...
            if self.frame_rate > 0:
                self.cam.MV_CC_SetFloatValue(
                    "AcquisitionFrameRate", float(self.frame_rate)
                )
            ...
            if ret == 0:
                try:
                    ...
                    if current_time - self.last_time >= 1.0:
                        ...
                        # print(f"相机原始帧率: {camera_fps:.2f} fps")  # 这就是相机输出的真实 FPS
                        self.frame_count = 0
