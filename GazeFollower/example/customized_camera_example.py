...
class PupilioCamera(Camera):
    ...
    def __init__(self):
        ...
        self.camera_utils = ctypes.CDLL(
            "customized_camera_native_library/lib_pupil_io_camera.dll", winmode=0
        )
    ...
    def _processing_frame(self):
        while self._camera_thread_running:
            ...
            img_1 = np.zeros(
                (self.source_img_height, self.source_img_width), dtype=np.uint8
            )
            img_2 = np.zeros(
                (self.source_img_height, self.source_img_width), dtype=np.uint8
            )
            ...
            if result == 1:
                ...
                frame0 = cv2.resize(
                    frame, (self.target_img_width, self.target_img_height)
                )
                try:
