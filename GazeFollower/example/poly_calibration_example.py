...
class PolyCalibration(Calibration):
    ...
    def __init__(self, model_save_path=""):
        ...
        if model_save_path:
            ...
            self.workplace_calibration_dir = pathlib.Path.home().joinpath(
        ...
        self.scaler_x_path = self.workplace_calibration_dir.joinpath(
            "poly_scaler_x.bin"
        )
        self.scaler_y_path = self.workplace_calibration_dir.joinpath(
            "poly_scaler_y.bin"
        )

        # Load pre-trained models and scalers if available
        if (
            self.cali_x_path.exists()
            and self.cali_y_path.exists()
            and self.scaler_x_path.exists()
            and self.scaler_y_path.exists()
        ):
            try:
    ...
    def calibrate(
        self, features: np.ndarray, labels: np.ndarray, ids=None
    ...
    def predict(
        self, features: np.ndarray, estimated_coordinate: Tuple[float, float]
    ...
    for _img in images:
        ...
        while not (got_key or (pygame.time.get_ticks() - t_start) >= max_duration):
            ...
            if gaze_info and gaze_info.status:
                ...
                pygame.draw.circle(
                    win, (0, 255, 0), (gx, gy), 50, 5
                )  # cursor for the left eye
