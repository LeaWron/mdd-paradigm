...
class GazeFollower:
    def __init__(
        self,
        camera: Camera = None,  # orignially WebCamCamera()
        ...
        if camera is None:
            from .camera.HikrobotCamera import HikvisionCamera
    ...
    def backend_name(screen):
        ...
        if screen_type == "":
            raise Exception(
                "Screen cannot be None. Please pass pygame window or psychopy window instance"
            )
        return screen_type
        ...
        if state == CameraRunningState.PREVIEWING:
            ...
            if face_info.status and face_info.can_gaze_estimation:
                ...
                cv2.rectangle(
                    ...
                    (relative_left_x + lw, relative_left_y + lh),
                    color=(255, 0, 0),
                    thickness=2,
                )
                ...
                cv2.rectangle(
                    ...
                    (relative_right_x + rw, relative_right_y + rh),
                    color=(0, 0, 255),
                    thickness=2,
                )
            # send the image frame to the screen
            self.camera_previewer_ui.update_images(
                frame, face_patch, left_eye_patch, right_eye_patch
            )
            ...
            if gaze_info.status and gaze_info.features is not None:
                calibrated, calibrated_gaze_coordinates = self.calibration.predict(
                    gaze_info.features, gaze_info.raw_gaze_coordinates
                )

                if not calibrated:
                    Log.e(
                        "No calibration model is available, please perform calibration"
                    )
                    raise Exception(
                        "No calibration model is available, please perform calibration"
                    )
                    ...
                    calibrated_gaze_coordinates = (
                        self._calibration_controller.convert_to_pixel(
                            calibrated_gaze_coordinates
                        )
                    )
                ...
                # do filter
                filtered_gaze_coordinates = self.gaze_filter.filter_values(
                    calibrated_gaze_coordinates
                )
                gaze_info.filtered_gaze_coordinates = filtered_gaze_coordinates
            self.dispatch_face_gaze_info(face_info, gaze_info)

        elif state == CameraRunningState.CALIBRATING:  # Logic for calibration
            if self._calibration_controller.calibrating:
                face_info = self.face_alignment.detect(timestamp, frame)
                gaze_info = self.gaze_estimator.detect(frame, face_info)
                self._calibration_controller.add_cali_feature(
                    gaze_info=gaze_info, face_info=face_info
                )
    ...
    def _gaze_info_2_string(gaze_info: GazeInfo, trigger):
        ...
        ret_str = (
            f"{gaze_info.timestamp},{gaze_info.raw_gaze_coordinates[0]},{gaze_info.raw_gaze_coordinates[1]},"
    ...
    def _write_sample(self, face_info, gaze_info):
        ...
        if isinstance(self._trigger, int):
            ...
            Log.e(
                "Trigger must be an integer, but you gave {}".format(
                    type(self._trigger)
                )
            )
            raise Exception(
    ...
    def _create_session(self, session_name: str):
        ...
        self._tmpSampleDataPath = _tmpDir.joinpath(
            f"em_{session_name}_{_timeString}.csv"
        )
