...
class CalibrationController:
    def __init__(
        ...
        self.normalized_point = generate_points()  # generate all points
    ...
    def update_position(self):
        ...
        self.progress = int(
            np.round(self._n_frame_added * 100 / self._n_frame_need_collect)
        )
    ...
    def add_cali_feature(self, gaze_info: GazeInfo, face_info: FaceInfo):
        ...
        if (time.time() - self._each_point_onset_time) >= self._prepare_time:
            if (
                gaze_info.status
                and (self._n_frame_added < self._n_frame_need_collect)
                and (face_info.left_eye_openness > self.eye_blink_threshold)
                and (face_info.right_eye_openness > self.eye_blink_threshold)
            ):
                if (
                    self._current_index != 0
                    and self._n_frame_added < self._n_frame_need_collect
                ):
                    self.feature_vectors[self._current_index - 1].append(
                        gaze_info.features
                    )
                    # self.label_vectors[self._current_index - 1].append(gaze_info.)
                    self.feature_ids[self._current_index - 1].append(
                    ...
                    if self.physical_screen_size:
                        ...
                        added_pos = px2cm(
                            (
                                ...
                                self.y * self.screen_size[1],
                            ),
                            self.cam_pos,
                            self.physical_screen_size,
                            self.screen_size,
                        )
                    else:
    ...
    def set_calibration_results(
        self, has_calibrated, mean_euclidean_error, labels, predictions
    ):
    ...
    def convert_to_pixel(self, raw_pos):
        ...
        if self.physical_screen_size:
            return cm2px(
                raw_pos, self.cam_pos, self.physical_screen_size, self.screen_size
            )
        else:
