...
class MediaPipeFaceAlignment(FaceAlignment):
    ...
    def detect(self, timestamp, image) -> FaceInfo:
        ...
        eye_left_xs = [
            face_landmarks[33].x - x_padding * scale,
            face_landmarks[133].x + x_padding * scale,
        ]
        ...
        eye_left_ys = [
            eye_left_y - eye_left_height * y_0,
            eye_left_y + eye_left_height * y_1,
        ]

        # get the X-coords for the right eye
        eye_right_xs = [
            face_landmarks[362].x - x_padding * scale,
            face_landmarks[263].x + x_padding * scale,
        ]
        # height of the eyebox is 0.75 of the width of the eyebox
        ...
        eye_right_ys = [
            eye_right_y - eye_right_height * y_0,
            eye_right_y + eye_right_height * y_1,
        ]
        ...
        if (
            (left_top_eye_left_point[0] <= 0)
            or (left_top_eye_left_point[1] <= 0)
            or (right_bottom_eye_left_point[0] >= image_width)
            or (right_bottom_eye_left_point[1] >= image_height)
        ):
            face_info.can_gaze_estimation = False
            return face_info

        if (
            (left_top_eye_right_point[0] <= 0)
            or (left_top_eye_right_point[1] <= 0)
            or (right_bottom_eye_right_point[0] >= image_width)
            or (right_bottom_eye_right_point[1] >= image_height)
        ):
            face_info.can_gaze_estimation = False
            return face_info
