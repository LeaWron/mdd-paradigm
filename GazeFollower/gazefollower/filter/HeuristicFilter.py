...
class HeuristicFilter(Filter):
    ...
    def do_filter(self, is_x, element):
        ...
        """
        raw = (
            self.raw_x if is_x else self.raw_y
        )  # Select the appropriate raw value list
        ...
        if len(raw) == self.look_ahead * 2 + 1:
            ...
            for next_val in range(1, self.look_ahead + 1):
                condition_one = (
                    raw[self.look_ahead - next_val] < raw[self.look_ahead]
                    and raw[self.look_ahead] > raw[self.look_ahead + next_val]
                )
                condition_two = (
                    raw[self.look_ahead - next_val] > raw[self.look_ahead]
                    and raw[self.look_ahead] < raw[self.look_ahead + next_val]
