...
class OneEuroFilter(Filter):
    ...
    def filter(self, value, timestamp=-1):
        ...
        d_value = (
            (value - self.x.last_raw_value()) * self.freq
            if self.x.has_last_raw_value()
            else 0.0
        )
        ed_value = self.dx.filter_with_alpha(
            d_value, self.alpha(self.d_cutoff)
        )  # Filter the derivative
        cutoff = self.min_cutoff + self.beta * abs(
            ed_value
        )  # Calculate adaptive cutoff
        return self.x.filter_with_alpha(
            value, self.alpha(cutoff)
