...
class MGazeNetGazeEstimator(GazeEstimator):
    ...
    def __init__(self, model_path: str = ""):
        ...
        if model_path == "":
            self.model_path = (
                pathlib.Path(__file__).parent.parent / "res/model_weights/base.mnn"
            )
        else:
        ...
        self.session = self.interpreter.createSession(
            {"precision": "normal", "numThread": 6, "backend": 0}
        )
        ...
        self.right_input_tensor = self.interpreter.getSessionInput(
            self.session, "right"
        )
    ...
    def detect(self, image, face_info: FaceInfo) -> GazeInfo:
        ...
        rect = np.array(
            [f_w, f_h, f_x, f_y, le_w, le_h, le_x, le_y, re_w, re_h, re_x, re_y],
            dtype=np.float32,
        ...
        left_patch = (
            cv2.resize(left_eye_patch, self.eye_size).astype(np.float32) / 255.0
        )
        ...
        left_tmp = MNN.Tensor(
            self.eye_input_format,
            MNN.Halide_Type_Float,
            left_patch,
            MNN.Tensor_DimensionType_Tensorflow,
        )

        right_tmp = MNN.Tensor(
            self.eye_input_format,
            MNN.Halide_Type_Float,
            right_patch,
            MNN.Tensor_DimensionType_Tensorflow,
