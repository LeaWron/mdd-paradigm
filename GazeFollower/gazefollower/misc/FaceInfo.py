...
class FaceInfo:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "Right Rect": _format_array(
                self.right_rect
            ),  # Format right eye bounding box
            "Left Rect": _format_array(self.left_rect),  # Format left eye bounding box
            "Face Landmarks": str(
                self.face_landmarks.shape
            ),  # Shape of landmarks array
            ...
            "Estimate Gaze": "Enable"
            if self.can_gaze_estimation
            else "Disable",  # Gaze estimation status
        }
