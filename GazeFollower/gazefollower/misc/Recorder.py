...
class Recorder:
    def __init__(
        ...
        subject_dir_format: str = "{subject_id}_{age}_{gender}_{wears_glasses}",
        ...
        if camera is None:
            ...
            if not isinstance(camera, Camera):
                raise TypeError(
                    f"Expected an instance of Camera, but got {type(camera).__name__}."
                )
            else:
                self.camera = camera
        ...
        self.subject_dir = os.path.join(
            str(self.dataset_dir), subject_dir_format.format(**self.participant_info)
        )
        ...
        self.feedback_sound = pygame.mixer.Sound(
            _audio_path
        )  # Replace with the path to your sound file

        # Load the arrow images
        _left_arrow_path = (
            Path(__file__).parent.parent.absolute() / "res/image/left_arrow.png"
        )

        self._arrow_image_size = 36
        # Load and resize the left arrow image
        self.left_red_arrow_image = pygame.transform.smoothscale(
            pygame.image.load(_left_arrow_path),
            (self._arrow_image_size, self._arrow_image_size),
        )

        # Create and resize the right arrow image by rotating the left arrow image
        self.right_red_arrow_image = pygame.transform.rotate(
            self.left_red_arrow_image, 180
        )

        # Function to change the color of the arrow image

        # Create left and right green arrow images
        self.left_green_arrow_image = self._change_arrow_color(
            self.left_red_arrow_image
        )  # Green color
        self.right_green_arrow_image = self._change_arrow_color(
            self.right_red_arrow_image
        )  # Green color
    ...
    def draw_guidance_text(self, screen, instruction_text):
        ...
        """
        text_surfaces = [
            self.guidance_font.render(line, True, self._color_white)
            for line in instruction_text
        ]

        total_text_height = (
            sum(text_surface.get_height() for text_surface in text_surfaces)
        ...
        for i, text_surface in enumerate(text_surfaces):
            text_rect = text_surface.get_rect(
                center=(
                    self.screen_width // 2,
                    start_y + i * (text_surface.get_height() + 10),
                )
            )
    ...
    def draw_arrows(self, screen, center: Tuple, direction: str, color: str):
        ...
        screen.blit(
            image,
            (center[0] - image.get_width() // 2, center[1] - image.get_height() // 2),
        )
...
class SmoothPursuitRecorder(Recorder):
    ...
    def __init__(
        self,
        camera=None,
        visible_rect: List = None,
        pursuit_params=None,
        dwelling_time: float = 2,
        dataset_dir: str = None,
        ...
        if visible_rect is None:
            _margin = 40
            self.visible_rect = [
                _margin / self.screen_width,
                _margin / self.screen_height,
    ...
    def start(self):
        ...
        response_df.to_excel(
            os.path.join(self.subject_dir, "participant_response_formal.xlsx"),
            index=False,
        )
        with open(
            os.path.join(self.subject_dir, "screen_size.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(
                {
    ...
    def _generate_arrows(duration, seed):
        ...
        while time_elapsed + 3 <= duration:
            ...
            arrows.append(
                (arrow_onset_time, arrow_offset_time, trial_end_time, direction)
            )
        return arrows
    ...
    def _new_session(self):
        ...
        self.current_point = [
            (self.visible_rect[0] + (self.visible_rect[2] - self.visible_rect[0]) // 2)
            * self.screen_width,
            (self.visible_rect[1] + (self.visible_rect[3] - self.visible_rect[1]) // 2)
            * self.screen_height,
        ]
    ...
    def draw(self, guidance_text):
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT or (
                    ...
                    event.key == pygame.K_f or event.key == pygame.K_j
                ):
                    ...
                    arrow_elapsed_time = (
                        time.time()
                        - self.start_time
                        - self.arrows[self._current_arrows_index][0]
                        - self.dwelling_time
                    )
                    if arrow_elapsed_time > 0 and (
                        self._current_arrows_index + 1 > len(self.responses)
                    ):
                        self.responses.append(
                            {
                                "arrow_direction": self.arrows[
            ...
            if self.elapsed_time > self.dwelling_time * 2 + self.duration:
                ...
                self.dwelling_time
                < self.elapsed_time
                < self.duration + self.dwelling_time
            ):
                time_elapsed_second = self.elapsed_time - self.dwelling_time
                x = self.amp_x * (
                    math.sin(
                        math.pi * 2 * self.freq_x * time_elapsed_second + self.phase_x
                    )
                    + 1
                )
                y = self.amp_y * (
                    math.sin(
                        math.pi * 2 * self.freq_y * time_elapsed_second + self.phase_y
                    )
                    + 1
                )
                ...
                self.draw_anti_aliased_circle(
                    screen=self.screen, point=self.current_point
                )
                ...
                if self._current_arrows_index < len(self.arrows):
                    (
                        ...
                        trial_end_time,
                        arrow_direction,
                    ) = self.arrows[self._current_arrows_index]
                ...
                self.elapsed_time < self.dwelling_time
                or self.elapsed_time > self.duration + self.dwelling_time
            ):
                self.current_point = (
        ...
        return pd.DataFrame(
            self.responses, columns=["arrow_direction", "response_key", "response_time"]
        )
    ...
    def draw_anti_aliased_circle(self, screen, point):
        ...
        circle_surface = pygame.Surface(
            (self._circle_size, self._circle_size), pygame.SRCALPHA
        )
        ...
        pygame.draw.circle(
            circle_surface,
            self._color_white,
            (self._circle_size // 2, self._circle_size // 2),
            self._circle_size // 2,
        )
        # Scale the surface slightly to make the edges smoother
        smooth_surface = pygame.transform.smoothscale(
            circle_surface, (self._circle_size, self._circle_size)
        )
        # Blit the circle onto the screen
        screen.blit(
            smooth_surface,
            (point[0] - self._circle_size // 2, point[1] - self._circle_size // 2),
        )
...
class NPointRecorder(Recorder):
    ...
    def __init__(
        ...
        self.pixel_points = tuple(
            [
                (point[0] * self.screen_width, point[1] * self.screen_height)
    ...
    def generate_point_directions(self):
        ...
        self.point_directions = ["left"] * (num_points // 2) + ["right"] * (
    ...
    def draw_breathing_effect(
        self, screen, center, outer_radius: int, inner_radius: int, elapsed_time: float
    ):
        """
        ...
        )  # Maximum expansion relative to inner circle
        if elapsed_time > pulse_period:
            return

        # Calculate the pulse offset to animate the gradient effect
        pulse_offset = math.sin(
            elapsed_time / pulse_period * math.pi / 2
        )  # Oscillates between 0 and 1
        current_radius = inner_radius + pulse_amplitude * (
            1 - pulse_offset
        )  # Decreases from max to min

        # Create a surface for the gradient effect with transparency
        gradient_surface = pygame.Surface(
            (2 * current_radius, 2 * current_radius), pygame.SRCALPHA
        )
        ...
        high_res_surface = pygame.Surface(
            (2 * high_res_radius, 2 * high_res_radius), pygame.SRCALPHA
        )

        # Draw concentric circles with varying intensity to create a gradient effect
        for i in range(
            high_res_radius, int(inner_radius * scale_factor), -2 * scale_factor
        ):
            color_intensity = int(
                128
                * ((i - inner_radius * scale_factor) / (pulse_amplitude * scale_factor))
            )
            gradient_color = (
                255,
                color_intensity,
                color_intensity,
                128,
            )  # Red gradient with varying alpha
            pygame.draw.circle(
                high_res_surface, gradient_color, (high_res_radius, high_res_radius), i
            )
        ...
        gradient_surface = pygame.transform.smoothscale(
            high_res_surface, (2 * int(current_radius), 2 * int(current_radius))
        )

        # Draw the gradient surface on the screen
        screen.blit(
            gradient_surface, (center[0] - current_radius, center[1] - current_radius)
        )
    ...
    def start(self):
        ...
        response_df.to_excel(
            os.path.join(self.subject_dir, "participant_response_practice.xlsx"),
            index=False,
        )
    ...
    def draw(self, guidance_text):
        ...
        while self.running:
                ...
                if event.type == pygame.QUIT or (
                    ...
                    if event.key == pygame.K_f or event.key == pygame.K_j:
                            ...
                            self.responses.append(
                                {
                                    ...
                                    "arrow_direction": self.point_directions[
                        ...
                        if self.point_elapsed_time >= self.dwelling_time:
                            ...
                            self.start_time = (
                                time.time()
                            )  # Reset the start time for the next point
            ...
            if self.current_point_index < len(points_need_to_draw):
                ...
                self.draw_breathing_effect(
                    self.screen,
                    current_point,
                    self._arrow_image_size,
