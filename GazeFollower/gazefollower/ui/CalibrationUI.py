...
class CalibrationUI(BaseUI):
    ...
    def draw_guidance(self, instruction_text):
        ...
        while self.running:
            ...
            # draw texts
            self.backend.draw_text_on_screen_center(
                instruction_text, self.font_name, self.font_size
            )
            # flip the screen
            self.backend.after_draw()

    def draw_cali_result(
        self, cali_controller: CalibrationController, model_fit_instruction: str
    ) -> bool:
        """
        ...
        while not cali_controller.cali_model_fitted:
            ...
            # draw texts
            self.backend.draw_text_on_screen_center(
                model_fit_instruction, self.font_name, self.font_size
            )
            # flip the screen
            self.backend.after_draw()
        ...
        while self.running:
            ...
            self.backend.draw_text_in_bottom_right_corner(
                text, self.font_name, self.row_font_size, text_color=self._color_black
            )
            ...
            if cali_controller.predictions is not None:
                for n, _ in enumerate(uni_p):
                    ...
                    self.backend.draw_circle(
                        avg_label[0], avg_label[1], 4, self._color_red
                    )
                    self.backend.draw_circle(
                        avg_prediction[0], avg_prediction[1], 4, self._color_green
                    )
    ...
    def draw(self, cali_controller: CalibrationController):
        ...
        while cali_controller.calibrating:
            ...
            target_x = int(
                np.round(cali_controller.x * self.backend.get_screen_size()[0])
            )
            target_y = int(
                np.round(cali_controller.y * self.backend.get_screen_size()[1])
            )
            draw_rect = (
                target_x - cali_img_size[0] // 2,
                target_y - cali_img_size[1] // 2,
                cali_img_size[0],
                cali_img_size[1],
            )
